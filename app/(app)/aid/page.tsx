    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-brand-600 text-sm">قدّم طلبك وتابع حالته خطوة بخطوة</p>
        </div>
        <Link href="/aid/new" className="btn-primary"><Plus size={16} /> طلب معونة جديد</Link>
      </div>

      <div className="card card-body bg-brand-50/40 border-brand-200">
      ...
      {loading && <div className="card card-body text-center text-brand-500">جاري التحميل...</div>}
      {!loading && list.length === 0 && (
        <div className="card card-body text-center py-12">
          <FileHeart className="mx-auto mb-3 text-brand-300" size={40} />
          <p className="text-brand-500 mb-4">لم تقدّم أي طلب معونة بعد</p>
          ...
          <Link key={r.id} href={`/aid/${r.id}`} className="card card-body hover:shadow-lg transition">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-bold text-brand-950 truncate">{r.title}</h3>
                <p className="text-sm text-brand-600 line-clamp-2 mt-1">{r.description}</p>
                <div className="text-xs text-brand-400 mt-2">قُدّم في {formatDate(r.created_at)}</div>
              </div>
              <div className="text-end shrink-0">
                {r.requested_amount && (
                  <div>
                    <div className="text-xs text-brand-500">المبلغ المطلوب</div>
                    <div className="font-bold text-brand-950">{formatMoney(r.requested_amount)}</div>
                  </div>
                )}
                {r.approved_amount && (
                  <div className="mt-2">
                    <div className="text-xs text-emerald-600">المعتمد</div>
                    <div className="font-bold text-emerald-700">{formatMoney(r.approved_amount)}</div>
                  </div>
                )}
              </div>
            </div>
