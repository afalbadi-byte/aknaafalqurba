  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-extrabold text-brand-950 dark:text-brand-50">دفعاتي</h1>
          <p className="text-brand-600 dark:text-brand-400 text-sm">سجل اشتراكاتك وتبرعاتك في الصندوق</p>
        </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="card card-body">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-lg bg-brand-100 dark:bg-brand-800 text-brand-700 dark:text-brand-300 flex items-center justify-center"><Wallet /></div>
            <div>
              <div className="text-xs text-brand-500 dark:text-brand-400">الإجمالي المعتمد</div>
              <div className="text-xl font-bold text-brand-950 dark:text-brand-50">{formatMoney(total)}</div>
            </div>
          </div>
        </div>
        <div className="card card-body">
          <div className="text-xs text-brand-500 dark:text-brand-400">عدد الدفعات</div>
          <div className="text-xl font-bold text-brand-950 dark:text-brand-50">{list.length}</div>
        </div>
        <div className="card card-body">
          <div className="text-xs text-brand-500 dark:text-brand-400">قيد المراجعة</div>
          <div className="text-xl font-bold text-amber-600 dark:text-amber-400">{list.filter(p => p.status === 'pending').length}</div>
        </div>
      </div>

      <div className="card">
        <div className="px-6 py-4 border-b border-brand-100 dark:border-brand-800">
          <h3 className="font-bold text-brand-950 dark:text-brand-50">سجل الدفعات</h3>
        </div>
        {loading && <div className="p-8 text-center text-brand-500 dark:text-brand-400">جاري التحميل...</div>}
        {!loading && list.length === 0 && (
          <div className="p-12 text-center">
            <Receipt className="mx-auto mb-3 text-brand-300 dark:text-brand-600" size={40} />
            <p className="text-brand-500 dark:text-brand-400 mb-4">لم تسجّل أي دفعة بعد</p>
        ...
        {!loading && list.length > 0 && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-brand-50/50 dark:bg-brand-800 text-brand-700 dark:text-brand-300 text-xs">
                <tr>
              ...
              <tbody className="divide-y divide-brand-50 dark:divide-brand-800">
                {list.map(p => (
                  <tr key={p.id} className="hover:bg-brand-50/30 dark:hover:bg-brand-800/40">
                    <td className="p-3 text-brand-700 dark:text-brand-300">{formatDate(p.created_at)}</td>
