...
export default function AdminReports() {
  ...
  if (!data) return <div className="text-center text-brand-500 py-12">جاري التحميل...</div>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="font-display text-2xl font-extrabold text-brand-950">التقارير المالية</h1>
        <p className="text-brand-600 text-sm">إحصاءات وتقارير شاملة عن أداء الصندوق</p>
      </div>
      ...
      <div className="card card-body">
        <h3 className="font-bold text-brand-950 mb-4">الإيرادات والمصروفات الشهرية</h3>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="card card-body">
          <h3 className="font-bold text-brand-950 mb-4">الإيرادات حسب النوع</h3>
        ...
        <div className="card card-body">
          <h3 className="font-bold text-brand-950 mb-4">المصروفات حسب التصنيف</h3>
          <div className="h-64 [direction:ltr]">
            <ResponsiveContainer>
              <PieChart>
      ...
      <div className="card">
        <div className="px-6 py-4 border-b border-brand-100"><h3 className="font-bold text-brand-950">أعلى المساهمين</h3></div>
            ...
            <tbody className="divide-y divide-brand-50">
              {data.top_contributors.map((c: any, i: number) => (
                <tr key={c.id} className="hover:bg-brand-50/30">
                  <td className="p-3 text-brand-500 font-bold">{i + 1}</td>
                  <td className="p-3 font-semibold text-brand-950">{c.full_name}</td>
                  <td className="p-3 text-brand-600">{c.branch || '—'}</td>
                  <td className="p-3 text-brand-700">{c.cnt}</td>
                  <td className="p-3 font-bold text-emerald-700">{formatMoney(c.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="card">
        <div className="px-6 py-4 border-b border-brand-100"><h3 className="font-bold text-brand-950">إجمالي مساهمات الأعضاء</h3></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-brand-50 text-brand-700 text-xs">
              <tr>
            ...
            <tbody className="divide-y divide-brand-50">
              {memberStats.map(m => (
                <tr key={m.id} className="hover:bg-brand-50/30">
                  <td className="p-3 font-semibold text-brand-950">{m.full_name}</td>
                  <td className="p-3 text-brand-600">{m.branch || '—'}</td>
                  <td className="p-3 text-brand-700">{m.payments_count}</td>
                  <td className="p-3 font-bold text-brand-950">{formatMoney(m.total_paid)}</td>
                </tr>
              ))}
            </tbody>
          </table>
