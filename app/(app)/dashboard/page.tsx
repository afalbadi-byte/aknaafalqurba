    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {isCommittee && stats?.fund && (
        <div className="card">
          <div className="px-6 py-4 border-b border-brand-100 flex items-center justify-between">
            <h3 className="font-bold text-brand-950">إحصاءات الصندوق</h3>
            <Link href="/admin/reports" className="text-sm text-brand-600 hover:underline">عرض التقارير الكاملة ←</Link>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-y lg:divide-y-0 divide-brand-100 [direction:rtl]">
            <Mini label="رصيد الصندوق"     value={formatMoney(stats.fund.balance)}        color="text-emerald-600" />
            <Mini label="إجمالي المحصّل"    value={formatMoney(stats.fund.total_collected)} />
            <Mini label="إجمالي المصروفات" value={formatMoney(stats.fund.total_expenses)}  color="text-red-600" />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="card lg:col-span-2">
          <div className="px-6 py-4 border-b border-brand-100 flex items-center justify-between">
            <h3 className="font-bold text-brand-950">آخر دفعاتك</h3>
            <Link href="/payments" className="text-sm text-brand-600 hover:underline flex items-center gap-1">
              عرض الكل <ChevronLeft size={14} />
            </Link>
          </div>
          {recent.length === 0 ? (
            <div className="p-8 text-center">
              <AlertCircle className="mx-auto mb-2 text-brand-300" />
              <p className="text-brand-500 text-sm mb-4">لم تسجّل أي دفعة بعد</p>
            ...
            <div className="divide-y divide-brand-50">
              {recent.map(p => (
                <div key={p.id} className="px-6 py-3 flex items-center justify-between">
                  <div>
                    <div className="font-semibold text-brand-950">{formatMoney(p.amount)}</div>
                    <div className="text-xs text-brand-500">{PAYMENT_METHODS[p.method]} · {formatDate(p.created_at)}</div>
                  </div>
        ...
        <div className="card">
          <div className="px-6 py-4 border-b border-brand-100 flex items-center justify-between">
            <h3 className="font-bold text-brand-950 flex items-center gap-2"><Newspaper size={18} /> آخر الأخبار</h3>
            <Link href="/news" className="text-sm text-brand-600 hover:underline">الكل ←</Link>
          </div>
          {news.length === 0 ? (
            <div className="p-6 text-center text-brand-500 text-sm">لا توجد أخبار</div>
          ) : (
            <div className="divide-y divide-brand-50">
              {news.map(n => (
                <Link key={n.id} href={`/news/${n.id}`} className="block px-6 py-3 hover:bg-brand-50/50">
                  <span className="badge badge-info text-[10px] mb-1">{NEWS_CATEGORIES[n.category]}</span>
                  <div className="font-semibold text-brand-950 line-clamp-2 text-sm">{n.title}</div>
                  <div className="text-[11px] text-brand-400 mt-1">{formatDate(n.published_at)}</div>
                </Link>
              ))}
            </div>
          )}
...
function Stat({ icon: Icon, label, value, sub, color }: any) {
  ...
  return (
    <div className="stat-card">
      ...
      <div className="text-2xl font-bold text-brand-950 mt-1">{value ?? 0}</div>
      {sub && <div className="text-xs text-brand-500 mt-1">{sub}</div>}
    </div>
  )
}

function Mini({ label, value, sub, color = 'text-brand-950' }: any) {
  return (
    <div className="p-5 text-center">
      <div className="text-xs text-brand-500 mb-1">{label}</div>
      <div className={`text-xl font-bold ${color}`}>{value}</div>
      {sub && <div className="text-[11px] text-brand-400 mt-1">{sub}</div>}
    </div>
  )
}
